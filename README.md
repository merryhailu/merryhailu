![White Minimalist Corporate Personal Profile LinkedIn Banner](https://github.com/user-attachments/assets/cc5bec1a-3041-46eb-bb2f-d9ecf99064c2)

## 👋 Hey there! I'm Meron Hailu.

I'm a passionate **Frontend Developer** with experience of crafting clean, responsive, and interactive user interfaces using modern web technologies.

---

### 🛠️ Tech Stack & Tools

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" alt="HTML5" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" alt="CSS3" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" alt="JavaScript" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" alt="React" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-plain.svg" alt="Tailwind CSS" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" alt="Git" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" alt="GitHub" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" alt="VSCode" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" alt="Figma" width="40" height="40"/>
</p>

---

### 💻 What I Do
- 🎨 Turn UI/UX designs into fully functional web interfaces  
- 📱 Build responsive layouts for all screen sizes  
- ⚙️ Integrate APIs and handle state management with React  
- 🔍 Debug, test, and optimize for performance  
- 🤝 Collaborate in agile environments and version-controlled projects

---

### 🎯 Goals
- 🚀 Contribute to open-source projects  
- 🌐 Master full-stack development  
- 🛠️ Build tools and apps that solve real-world problems

---

### 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=your-github-username&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=your-github-username&theme=tokyonight" />
</p>

---

### 📫 Let's Connect

- 💼 [LinkedIn](https://www.linkedin.com/in/meron-hailu-/)
- 🌐 [Portfolio](https://github.com/merryhailu/meronhailu_portfolio)
- 📧 meronhailu1044@gmail.com

---


